...
export function createOpenFigiResolver(options: OpenFigiResolverOptions = {}) {
  ...
  async function map(
    requests: readonly OpenFigiMappingRequest[],
  ): Promise<readonly (readonly OpenFigiMatch[])[]> {
    ...
    if (requests.length > maxJobs) {
      throw new ProviderError(
        ...
        `OpenFIGI accepte au plus ${maxJobs} jobs par requête dans ce mode`,
      );
    }
    ...
    try {
      ...
      if (!Array.isArray(payload) || payload.length !== requests.length) {
        throw new ProviderError(
          "MALFORMED_RESPONSE",
          OPENFIGI_PROVIDER_ID,
          "Réponse OpenFIGI non alignée avec les jobs envoyés",
        );
      }
  ...
  return {
    ...
    async byTicker(ticker: string, micCode?: string): Promise<readonly OpenFigiMatch[]> {
      return (
        (
          await map([
            { idType: "TICKER", idValue: ticker, ...(micCode === undefined ? {} : { micCode }) },
          ])
        )[0] ?? []
      );
    },
