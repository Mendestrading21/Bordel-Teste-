...
export type LiveProviderConfig = {
  ...
  readonly providers: Readonly<Record<string, { enabled: boolean; mode: LiveProviderMode; apiKeyPresent: boolean }>>;
};
...
export function readLiveProviderConfig(env: NodeJS.ProcessEnv = process.env): LiveProviderConfig {
  const marketDataMode = env.MARKET_DATA_MODE === "demo" || env.MARKET_DATA_MODE === "live"
...
export function validateLiveProviderConfig(config: LiveProviderConfig): readonly string[] {
  ...
  if (config.marketDataMode === "live") {
    ...
    if (enabled.length === 0) issues.push("MARKET_DATA_MODE=live mais aucun fournisseur n'est activé");
